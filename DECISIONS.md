...
El resultado es que la curva de valor está invertida: cuanto más grande e irreversible es la tarea, menos protecciones tiene. El field report de 2026-08-08 (956 borrados irreversibles en una sola sesión) corrió con estrictamente menos garantías que un solo `browser.click`.
...
- Bajo el techo actual de 60s, un loop de drenado con throttle (un click con `settle_ms: 0`, `while_found` y `delay_ms: 250`) entra en **46 iteraciones por llamada** — medido: 46 acepta, 47 rechaza con 61s. Sin throttle y con settle apagado son ~116; con settle por defecto, ~23. O sea que la ganancia de `repeat` NO es de throughput frente a un loop en `evaluate` (las 956 borradas del field report serían ~21 llamadas, no ~5): es de SEGURIDAD — input confiable, guard de oclusión, registro por iteración y rechazo up-front. El techo de throughput es justo lo que levanta la fase 6, y no eliminando `MAX_FLOW_TIMEOUT_MS`: un flow síncrono lo sigue respetando, y lo que la fase 6 agrega es una forma de ejecución —`detach: true`— a la que esa regla no le aplica, acotada por su propio presupuesto y por el techo de 30 minutos (ver el punto siguiente).
- ~~Cuando llegue la fase 6 hay que volver acá y revisar `MAX_FLOW_TIMEOUT_MS`: la restricción que codifica deja de valer.~~ **Revisado en v0.27.0 (fase 6 entregada).** El cap de 60s no se elimina: cambia de naturaleza. Dejó de ser una consecuencia de no tener lifecycle y pasó a ser una regla de economía de turnos — cuánto tiempo el bridge deja parkeada UNA tool call, con la misma lógica que `browser.wait_for_tab` o `browser.evaluate`. Un flow síncrono lo sigue respetando y se sigue rechazando up-front, porque nada cancela un flow solo porque el bridge dejó de esperarlo, y una respuesta descartada a mitad de un flow que sigue ejecutando es justo el escenario que produce acciones duplicadas al reintentar. Un flow `detach: true` no parkea nada, así que la regla no le aplica: queda acotado por `MAX_DETACHED_FLOW_TIMEOUT_MS` (30 minutos), que es a la vez el techo contra el que se valida su presupuesto estático up-front y el deadline de reloj de pared con el que se auto-cancela reportando `stopped_by: 'expired'`. Lo que NO se relajó: el presupuesto worst-case se sigue calculando y `repeat.max_iterations` sigue siendo obligatorio. "Incognoscible" es precisamente lo que no puede ser la duración de un loop que sigue clickeando sin ningún agente conectado. La circularidad del spec queda rota sin quedarse sin techo.
...
- ~~Queda abierto y hay que resolverlo **antes** de implementar la fase 6:~~ **Resuelto en v0.27.0 por la opción conservadora.** Al lanzar un flow detached se escribe un registro `detached-flow:<flow_id>` en `chrome.storage.session` ANTES del primer step. Al arrancar el service worker, cualquier registro que siga en `running` es por definición huérfano (el loop vivía en la memoria del worker muerto): se reescribe como `failed` / `worker-terminated`, se anota en el historial del popup y se emite su evento resumen en la próxima registración de la pestaña. **Nunca se reanuda.** Un flow detached existe para hacer trabajo masivo irreversible; reanudar uno cuyo progreso real murió con el worker duplicaría acciones sobre un prefijo desconocido. Fallar fuerte es la única opción honesta, y un flow que _parece_ `running` mientras nada lo ejecuta es peor que ambas. El manifiesto parcial de una corrida muerta es irrecuperable —vivía en el closure del runner— y se reporta como `manifest_truncated: true` en vez de devolver un array vacío que se leería como "no pasó nada"; la alternativa (escribir el manifiesto por step) serían cientos de escrituras a storage por corrida solo para mantener tibio un contador. No se implementó keep-alive: mantener vivo el worker a propósito cambia el problema de "puede morir" a "no debería morir", y la garantía que importa es que nunca mienta sobre su estado.
