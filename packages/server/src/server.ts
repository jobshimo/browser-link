...
backed by a local SQLite DB. The data dir resolves per-OS via env-paths
(\$XDG_DATA_HOME/browser-link on Linux, ~/Library/Application Support/browser-link
on macOS, %APPDATA%/browser-link on Windows). Override with
\$BROWSER_LINK_DATA_DIR. The map is private and per-machine; never persisted in any repo.
...
/**
 * Start the MCP server (stdio transport) and the WebSocket bridge for the
 * Chrome extension. Resolves when the server is ready to handle requests.
 * This is the entry point invoked by MCP clients (Claude Code, OpenCode, …).
 */
export async function runServer(): Promise<void> {
  ...
  const assignTabId = (): string => `tab_${++tabIdCounter}`;
  ...
  const requireTabId = (args: unknown): string => {
