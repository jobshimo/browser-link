...
function body(): string {
  return [
    ...
    '```',
    '',
    '- Hundreds of iterations in ONE round trip, and every click is still',
    '  trusted CDP input with the occlusion guard and a recovery snapshot.',
    '  An `evaluate` loop has NONE of that — `el.click()` is',
    ...
    '  exceeds 60s the flow is rejected up front — lower `max_iterations`',
    '  and run the flow again; each call picks up where the last left off',
    '  when `while_found` still matches.',
    ...
    '  `while_found` / `would_start` per repeat.',
    '',
    '### LONG-RUNNING WORK — a timed-out `evaluate` KEEPS RUNNING',
    '',
    'Reach for this only when `flow` + `repeat` genuinely cannot express',
    'the work (it needs branching, or arbitrary JS per item).',
    '',
